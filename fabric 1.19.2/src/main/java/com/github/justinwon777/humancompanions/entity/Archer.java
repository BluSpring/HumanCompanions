...
public class Archer extends AbstractHumanCompanionEntity implements RangedAttackMob {
    ...
    public void performRangedAttack(LivingEntity p_32141_, float p_32142_) {
        ...
        ItemStack itemstack = this.getProjectile(this.getItemInHand(item instanceof BowItem ? InteractionHand.MAIN_HAND : InteractionHand.OFF_HAND));
        AbstractArrow abstractarrow = this.getArrow(itemstack, p_32142_);
        if (this.getMainHandItem().getItem() instanceof BowItem bowItem) {
            if (bowItem instanceof CustomArrowItem customArrowItem)
                abstractarrow = customArrowItem.customArrow(abstractarrow);
        }
